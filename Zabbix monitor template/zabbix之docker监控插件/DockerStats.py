...
class ExecFunByZabbixRequest(object):
    ...
    def GetCpuTotalUsage(self,Cid):
        ...
        if self.DataSourceTag == "DockerApi":
            ...
            previousCPU = RetDataDict["precpu_stats"]["cpu_usage"]["total_usage"]
            previousSystem = RetDataDict["precpu_stats"]["system_cpu_usage"]

            nowCPU = RetDataDict["cpu_stats"]["cpu_usage"]["total_usage"]
            nowSystem = RetDataDict["cpu_stats"]["system_cpu_usage"]

            cpuDelta = float(nowCPU - previousCPU)
            systemDelta = float(nowSystem - previousSystem)

            if systemDelta > 0.0 and cpuDelta > 0.0:
                CpuTotalUsage = "%.2f"%((cpuDelta / systemDelta) * float(len(RetDataDict["cpu_stats"]["cpu_usage"]["percpu_usage"])) * 100)
